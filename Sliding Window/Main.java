...
public class Main {
    ...
    public static long cses(long n) {
        if (n % 2 == 0)
            return n / 2;
        else
            return n * 3 + 1;
    ...
    public static String helpMagic(int k, int r1, int c1, int r2, int c2, int[][] grid) {
    ...
    public static long helper(long[] arr, long[][] dp, int n, int defeatedCount) {
        if (n == arr.length) {
            return 0;
        }

        if (dp[n][defeatedCount % 2] != -1)
            return dp[n][defeatedCount % 2];

        long notDefeat = helper(arr, dp, n + 1, defeatedCount);

        long defeat = arr[n];
        if ((defeatedCount + 1) % 2 == 0) {
            defeat += arr[n];
        }
        defeat += helper(arr, dp, n + 1, defeatedCount + 1);

        return dp[n][defeatedCount % 2] = Math.max(notDefeat, defeat);
    }
